...
from utils.chart_utils.indicators import get_SMA,get_borders
...
class ST9(VisualTraider_v2):
    ...
    def _get_keys(self, img, region) -> dict:
        ...
        top_line,bottom_line = get_borders(region)
        ...
        if self.mode != 1:
            ...
            cv2.line(chart,top_line[0],top_line[1],(200,0,200),2)
            cv2.line(chart,bottom_line[0],bottom_line[1],(200,200,200),2)
        return keys
    ...
    def _get_action(self,keys:Keys):
        ...
        if self.have_pos_s:
            if keys.cur_price > keys.take or keys.cur_price < keys.stop_short:
                return 'close_short'
        if keys.go and keys.bottom_limit > keys.cur_price > keys.top_limit:
