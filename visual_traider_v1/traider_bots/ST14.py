...
class ST14(VisualTraider_v2):
    ...
    def _get_keys(self, img, region) -> dict:
        ...
        if len(ups) > 1:
            for i in range(len(ups)-1,0,-1):
                if ups[i][1]-volatility[-1][1] < cur_price[1]:
                    creek = ups[i][1]
                    break
        if len(downs) > 1:
            for i in range(len(downs)-1,0,-1):
                if downs[i][1]+volatility[-1][1] > cur_price[1]:
                    ice = downs[i][1]
                    break
    ...
    def _get_action(self,keys:Keys):
        ...
        # range
            ...
            if keys.cur_price > keys.ice-keys.volatility:
                return 'long'
            if keys.cur_price < keys.creek+keys.volatility:
                return 'short'
