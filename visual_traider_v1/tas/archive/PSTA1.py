...
class PSTA1(BaseTA):  
    def get_keys(self, img) -> KeysW:
        ...
        points = self.trader._get_points(half_bars)
        x,y = self.trader._get_xy(points)
        trend,top_trend,bottom_trend,slope = self.trader._get_trend_lines(x,y)
        points = self.trader._get_points(half_bars[-len(half_bars)//4:])
        x,y = self.trader._get_xy(points)
        trend_sm,top_trend_sm,bottom_trend_sm,slope_sm = self.trader._get_trend_lines(x,y)
        ...
        self.trader.stop_long = stop_long[1]
        self.trader.stop_short = stop_short[1]
        keys = KeysW(
            cur_price[1],
            bbd_sm[-1][1],
            bbu_sm[-1][1],
        ...
        if self.trader.mode != 1:
            ...
            # cv2.putText(chart,"FS: " +str(self.trader.free_stop_l,self.trader.free_stop_s),(0,70),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
